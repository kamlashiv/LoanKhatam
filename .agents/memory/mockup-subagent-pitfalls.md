...
4. **Unclosed JSX tags after an edit pass.** A polish/refinement pass by a subagent can drop a closing `</div>` (e.g. an opened wrapper before `</CardContent>`), producing `TS17008: JSX element 'div' has no corresponding closing tag`. The preview URL still returns **200** and the subagent reports success — only `tsc --noEmit` catches it.
   **How to apply:** always run `pnpm --filter @workspace/mockup-sandbox run typecheck` (or `cd artifacts/mockup-sandbox && npm run typecheck`) after subagents finish, before screenshotting. Treat typecheck as mandatory, not optional, for mockups too.

**Verification path that works:** restart the mockup-sandbox workflow → grep for escaped literals → **typecheck** → screenshot every variant → architect review for data consistency → fix → re-screenshot → `presentArtifact`. Do NOT `suggestDeploy` (mockup sandbox isn't deployable).

**Screenshot caching gotcha:** the external_url screenshot service caches by exact URL. After editing + restarting, a re-screenshot of the same preview URL can return a pixel-identical STALE image, making a real fix look un-applied. Bust it by appending a query param (`?v=2`) to the preview URL to force a fresh capture.
