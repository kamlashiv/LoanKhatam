...
**Why:** task required loan figures to derive from real loans and the EMI analyzer to READ/WRITE profile income/expenses rather than own them, so no screen can drift from the actual loan ledger.
...
- Domain tension: DB loans are money *lent out*, but task-67 treats them as the user's debts for the planner/strategy. Follow the task, not intuition.
