...
const AUTH_PATH = path.join(__dirname, "../../admin_auth.json");
...
function writeAuth(username: string, password: string) {
  try {
    fs.writeFileSync(AUTH_PATH, JSON.stringify({ username, password }, null, 2), "utf-8");
  } catch (e) {
...
router.post("/admin/credentials", authorizeAdmin, (req, res) => {
